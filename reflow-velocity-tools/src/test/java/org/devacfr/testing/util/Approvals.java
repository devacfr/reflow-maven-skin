...
public final class Approvals {
  ...
  public static void verify(@Nonnull final Path location,
    ...
    @Nullable final String extension,
    @Nullable final String outExtension) {
    String actual = requireNonNull(getActualResource(location, testClass, testName, extension));
    if (transform != null) {
      actual = transform.apply(actual);
    }
    final String expected = getExpectedResource(location, testClass, testName, outExtension, null);
  ...
  public static class IsEqualCompressingWhiteSpace extends TypeSafeMatcher<String> {
    ...
    // TODO: Replace String with CharSequence to allow for easy interoperability
    // between
    ...
    private String getInternalDiff(final String expected, final String actual) {
      ...
      try {
        ...
        return diffs.toUnifiedDiff("actual", "expected", new StringReader(actual), new StringReader(expected), 10);
      } catch (final IOException e) {
        throw new RuntimeException(e.getMessage(), e);
      }
    }
