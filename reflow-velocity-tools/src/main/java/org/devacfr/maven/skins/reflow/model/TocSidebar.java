...
public class TocSidebar extends Toc<TocSidebar> {
  ...
  public TocSidebar(final @Nonnull ISkinConfig config) {
    ...
    if (this.isEnabled()) {
      this.addCssOptions("toc-sidebar-enabled");
    }
    if (isExpanded()) {
      this.addCssOptions("toc-sidebar-expanded");
    }
    if (isAutoExpandable()) {
      this.addCssOptions("toc-sidebar-autoexpandable");
    }
    if (isFixed()) {
