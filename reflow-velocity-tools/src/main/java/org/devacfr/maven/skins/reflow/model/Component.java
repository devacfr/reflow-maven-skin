...
public abstract class Component {
   ...
   */
  protected Component(@Nonnull final ISkinConfig config) {
  ...
  public final String getCssOptions() {
    return concat(this.cssOptions, concat(FluentIterable.from(children).transform(component -> component.cssOptions)))
        .join(Joiner.on(' '));
  }
   ...
   */
  protected String onPreRender(@Nonnull final String bodyContent) {
    final StringBuilder str = new StringBuilder(bodyContent);
    this.children.forEach(component -> {
      final String content = str.toString();
      str.setLength(0);
      str.append(component.onPreRender(content));
    });
    return str.toString();
  }
